//
// Created by urmenus on 11/9/25.
//

#ifndef MYOPENCVPROJECT_COLORDETECTOR_H
#define MYOPENCVPROJECT_COLORDETECTOR_H
...
class ColorDetector {
  ...
  public:
	  ...
	  cv::Mat operator()(const cv::Mat &image) {
		  ...
		  // add the 3 channels (saturation might occur here)
	...
	cv::Point findSeedPoint(const cv::Mat& binaryMask) {
	  	for (int y = 0; y < binaryMask.rows; ++y) {
	  		for (int x = 0; x < binaryMask.cols; ++x) {
	  			if (binaryMask.at<uchar>(y, x) == 255) {
	  				return cv::Point(x, y); // first white pixel found
	  			}
	  		}
	  	}
	  	return cv::Point(-1, -1); // not found
	  }

};

#endif //MYOPENCVPROJECT_COLORDETECTOR_H